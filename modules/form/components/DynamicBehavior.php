...
class DynamicBehavior extends Behavior
{
	...
	public function __call($name, $params)
	{
		...
		if(substr($name, 0, strlen('set')) === 'set') {
			$varName = substr($str, 3);
			if(isset($this->vars[$varName]) && isset($params[0])) return $this->vars[$varName] = $params[0];
		}
		if(array_key_exists($name, $_methods)) {
			...
			if(@eval('return true;' . $code)) return @eval($code); //evaluate code parsing and then call code	
